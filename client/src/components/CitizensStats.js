...
const CitizensStats = () => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [statsRes, censusRes, summaryRes] = await Promise.all([
        axios.get('/api/citizen-stats', { headers: { Authorization: `Bearer ${token}` } }),
  ...
  return (
    <div className="tab-section">
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {[
          { key: 'overview', label: '📊 نظرة عامة', desc: 'إحصائيات العمال' },
          { key: 'census', label: '📋 التعداد السكاني', desc: 'استمارات التعداد' },
        ].map(v => (
          <button key={v.key} onClick={() => setActiveView(v.key)}
            style={{ padding: '0.6rem 1.2rem', borderRadius: '12px', border: '1px solid', borderColor: activeView === v.key ? 'var(--primary)' : 'rgba(99,102,241,0.2)', background: activeView === v.key ? 'rgba(99,102,241,0.2)' : 'rgba(30,41,59,0.5)', color: activeView === v.key ? 'var(--primary-light)' : 'var(--gray-light)', cursor: 'pointer', fontSize: '0.85rem', fontFamily: 'inherit', fontWeight: '600', transition: 'all 0.2s', flex: '1', minWidth: '150px' }}>
            <div>{v.label}</div>
            <div style={{ fontSize: '0.7rem', opacity: 0.7 }}>{v.desc}</div>
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, display: 'flex', gap: '0.5rem' }}>
              ...
              <button className="btn-primary btn-sm" onClick={() => { setCensusPage(1); fetchStats(); }}>بحث</button>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={() => { setShowCensusForm(false); setEditingCensus(null); }}>
          <div className="modal-content" style={{ maxWidth: '800px', maxHeight: '90vh', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <CensusForm
              ...
              onSave={() => { setShowCensusForm(false); setEditingCensus(null); fetchStats(); }}
